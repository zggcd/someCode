...
function dhtmlXTreeObject(htmlObject, width, height, rootId){
	...
	/* 
	   新增属性，默认为true，用来设置复选框状态是否向上传递。
	   当设置复选框threeStateCheckboxes属性为true，checkIsUpPass未设置或设为true时，复选框状态向上传递；
	   当threeStateCheckboxes为true，checkIsUpPass为false，则复选框状态不向上传递。
	   即：勾中树上某个子节点，其父节点也被勾选；但去掉子节点的勾，父节点依然被勾选。		   
	 */
	this.checkIsUpPass = true;
	...
	this.allTree=this._createSelf();this.allTree.appendChild(this.htmlNode.htmlNode);
	...
	//新增属性事件，给checkIsUpPass赋值，设置复选框状态是否向上传递
	dhtmlXTreeObject.prototype.enableCheckIsUpPass=function(flag){this.checkIsUpPass=flag;};
    ...
    if (z){
	 ...
	 dhtmlXTreeObject.prototype._escape=function(str){switch(this.utfesc){case "none":
 ...
 parentObject.htmlNode.childNodes[0].appendChild(tr);if ((beforeNode)&&(!beforeNode.span)) beforeNode=null;if (this.XMLsource)if ((childs)&&(childs!=0)) Nodes[Count].XMLload=0;else Nodes[Count].XMLload=1;Nodes[Count].tr=tr;tr.nodem=Nodes[Count];if (parentObject.itemId==0)tr.childNodes[0].className="hiddenRow";if ((parentObject._r_logic)||(this._frbtr))
 this._setSrc(Nodes[Count].htmlNode.childNodes[0].childNodes[0].childNodes[1].childNodes[0],this.imPath+this.radioArray[0]);if (optionStr){var tempStr=optionStr.split(",");for (var i=0;i<tempStr.length;i++){switch(tempStr[i])
 {case "SELECT": this.selectItem(itemId,false);break;case "CALL": this.selectItem(itemId,true);break;case "CHILD": Nodes[Count].XMLload=0;break;case "CHECKED":
 if (this.XMLloadingWarning)this.setCheckList+=this.dlmtr+itemId;else
 this.setCheck(itemId,1);break;case "HCHECKED":
 this._setCheck(Nodes[Count],"unsure");break;case "OPEN": Nodes[Count].openMe=1;break;};};};if (!this.XMLloadingWarning){if ((this._getOpenState(parentObject)<0)&&(!this._hAdI)) this.openItem(parentObject.id);if (beforeNode){this._correctPlus(beforeNode);this._correctLine(beforeNode);};this._correctPlus(parentObject);this._correctLine(parentObject);this._correctPlus(Nodes[Count]);if (parentObject.childsCount>=2){this._correctPlus(Nodes[parentObject.childsCount-2]);this._correctLine(Nodes[parentObject.childsCount-2]);};if (parentObject.childsCount!=2)this._correctPlus(Nodes[0]);if (this.tscheck)this._correctCheckStates(parentObject);if (this._onradh){if (this.xmlstate==1){var old=this.onXLE;this.onXLE=function(id){this._onradh(itemId);if (old)old(id);};}else
 this._onradh(itemId);};};return Nodes[Count];};
 ...
 this._setNextPageSign(temp,npl+1*(start||0),level,node);break;
 };
 ...
 newNode.span.setAttribute("style",newNode.span.getAttribute("style")+";"+style);if (aradio)newNode._r_logic=true;
 
 if (nocheckbox){
	 //newNode.span.parentNode.previousSibling.previousSibling.childNodes[0].style.display='none';
	 //当树设置显示复选框（checkBoxes="1"），某个节点不显示复选框时（nocheckbox="1"），要将img的上级节点隐藏，而不是将img隐藏，
	 //否则在google浏览器下，放复选框位置的宽度仍保留着，造成节点前面的空白。
	 newNode.span.parentNode.previousSibling.previousSibling.childNodes[0].parentNode.style.display='none';
 ...
 if (disheckbox){
	...
	//else里面的代码是升级新增加的，只有当checkbox设置有三态并且不向上传递时才执行这段代码。并且当树展现的时候，若只选择父没选择子，子不会被选中，而父是半选中状态。
	//升级之前，因为没有不向上传递的功能，所有这点就没考虑到。
	if((achecked==null || achecked=="") && newNode.parentObject.checkstate!=0){
		dhtmlObject._correctCheckStates(newNode.parentObject);
		dhtmlObject._setCheck(newNode,false);
	}
 };
...
dhtmlXTreeObject.prototype._HideShow=function(itemObject,mode){
	 this.openerID = itemObject.id;
	if ((this.XMLsource)&&(!itemObject.XMLload)) {
		if (mode==1)return;
	    itemObject.XMLload=1;
		this._loadDynXML(itemObject.id,null,itemObject.checkstate);
		return;
	};
	...
	if (Count>1){
		if ( ( (Nodes[1].style.display!="none")|| (mode==1) ) && (mode!=2) ) {
			...
			this.allTree.childNodes[0].border = "0";nodestyle="none";
	     }else nodestyle="";
 ...
 that.callEvent("onRightClick",[obj.id,e]);if (obj.actionHandler)obj.actionHandler(obj.id,lastId);else that.callEvent("onClick",[obj.id,lastId]);};};
 ...
 dhtmlXTreeObject.prototype._correctCheckStates=function(dhtmlObject){
	 ...
	 if ((flag1)&&(flag2)) {
		 ...
		 /* 在此多加了一个条件，checkIsUpPass为true时，才执行以前的代码，即设置复选框状态向上传递；
		         否则，将不做任何操作，即复选框状态不向上传递。
		 */
		 if(this.checkIsUpPass)		//新加代码
			 this._setCheck(dhtmlObject,false);   //原始代码
		 else    //新加代码
			this._setCheck(dhtmlObject,"unsure");  //新加代码
	 }else {
 ...
 dhtmlXTreeObject.prototype.onCheckBoxClick=function(e) {if (this.parentObject.dscheck)return true;if (this.treeNod.tscheck)if (this.parentObject.checkstate==1)this.treeNod._setSubChecked(false,this.parentObject);else this.treeNod._setSubChecked(true,this.parentObject);else
...
dhtmlXTreeObject.prototype.setOnCheckHandler=function(func){this.attachEvent("onCheck",func);};dhtmlXTreeObject.prototype.setOnOpenHandler=function(func){this.attachEvent("onOpenStart",func);};dhtmlXTreeObject.prototype.setOnOpenStartHandler=function(func){this.attachEvent("onOpenStart",func);};dhtmlXTreeObject.prototype.setOnOpenEndHandler=function(func){this.attachEvent("onOpenEnd",func);};dhtmlXTreeObject.prototype.setOnDblClickHandler=function(func){this.attachEvent("onDblClick",func);};dhtmlXTreeObject.prototype.openAllItems=function(itemId)
 ...
 {state=convertStringToBoolean(state);if ((this.tscheck)&&(this.smcheck)) this._setSubChecked(state,sNode);else this._setCheck(sNode,state);};if (this.smcheck)this._correctCheckStates(sNode.parentObject);};dhtmlXTreeObject.prototype._setCheck=function(sNode,state){if (((sNode.parentObject._r_logic)||(this._frbtr))&&(state))
...
{this.dhx_SeverCatcherPath="";this.attachEvent = function(original, catcher, CallObj)
 {CallObj = CallObj||this;original = 'ev_'+original;if ( ( !this[original] )|| ( !this[original].addEvent ) ) {var z = new this.eventCatcher(CallObj);z.addEvent( this[original] );this[original] = z;};return ( original + ':' + this[original].addEvent(catcher) );};this.callEvent=function(name,arguments){if (this["ev_"+name])return this["ev_"+name].apply(this,arguments);return true;};this.checkEvent=function(name){if (this["ev_"+name])return true;return false;};this.eventCatcher = function(obj)
 {var dhx_catch = new Array();var m_obj = obj;var func_server = function(catcher,rpc)
 {catcher = catcher.split(":");var postVar="";var postVar2="";var target=catcher[1];if (catcher[1]=="rpc"){postVar='<?xml version="1.0"?><methodCall><methodName>'+catcher[2]+'</methodName><params>';postVar2="</params></methodCall>";target=rpc;};var z = function() {return true;};return z;};var z = function()
