	...
	try {
		...
		if (retHashtable != null){
			...
			if (orgTreeList != null){
				...
				retHashtable = (Hashtable) business.invoke("baf_operateGroupBI", "getOrgByGroupId", hashtable);
				List orgIdList = null;
				if(retHashtable != null){
					orgIdList = (List)retHashtable.get("getOrgByGroupId");
				}
				int k = 0;
			    for (int i=0;i<orgTreeList.size();i++) {
					...
					//将已授权的组织复选框选中
					if(orgIdList != null && orgIdList.contains(vo.getObjectid())){
						builder.append(" checked = \"1\" "); 
					}
					
					builder.append(" >\n");
					
					builder.append("<userdata name=\"isSelect\">");
