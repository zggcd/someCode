...
if ( !this.ParentWindow ){
	...
	arr.push("        <tr><td align='center' valign='top'><div style='position:relative;overflow:auto;width:"+this.Width+"px;height:"+this.Height+"px;'>");
	...
	arr.push("            <div id='_DialogButtons_"+this.ID+"' style='text-align:center; border-top:#dadee5 1px solid; padding:8px 20px; background-color:#f6f6f6;'>");
	arr.push("           	<input id='_ButtonOK_"+this.ID+"'  type='button' style='background:#d9eefd;border:#7f9db9 1px solid;margin:0;padding:0;' value='确 定'>");
	arr.push("           	<input id='_ButtonCancel_"+this.ID+"'  type='button' style='background:#d9eefd;border:#7f9db9 1px solid;margin:0;padding:0;' onclick=\"Dialog.getInstance('"+this.ID+"').close();\" value='取 消'>");
	arr.push("            </div></td></tr>");
	arr.push("      </table><a href='#;' onfocus='$m(\"_forTab_"+this.ID+"\").focus();'></a></td>");
	...
	var div = pw.$m("_DialogDiv_"+this.ID);
	if(!div){
	...
	if(this.innerElementId){
		var innerElement=$m(this.innerElementId);
		...
		if(isIE){
			...
			pw.$m("_Covering_"+this.ID).parentNode.appendChild(fragment)
		}else{
			pw.$m("_Covering_"+this.ID).parentNode.appendChild(innerElement)
		}
	}
	pw.$m("_DialogDiv_"+this.ID).DialogInstance = this;
	if(this.URL)
		pw.$m("_DialogFrame_"+this.ID).DialogInstance = this;
	pw.Drag.init(pw.$m("_draghandle_"+this.ID),pw.$m("_DialogDiv_"+this.ID));//注册拖拽方法
	if(!isIE){
		pw.$m("_DialogDiv_"+this.ID).dialogId=this.ID;
		pw.$m("_DialogDiv_"+this.ID).onDragStart = function(){$m("_Covering_"+this.dialogId).style.display=""}
		pw.$m("_DialogDiv_"+this.ID).onDragEnd = function(){$m("_Covering_"+this.dialogId).style.display="none"}
	}

	this.OKButton = pw.$m("_ButtonOK_"+this.ID);
	this.CancelButton = pw.$m("_ButtonCancel_"+this.ID);

	//显示标题图片
	if(this.ShowMessageRow){
		$E.show(pw.$m("_MessageRow_"+this.ID));
		if(this.MessageTitle){
			pw.$m("_MessageTitle_"+this.ID).innerHTML = this.MessageTitle;
		}
		if(this.Message){
			pw.$m("_Message_"+this.ID).innerHTML = this.Message;
		}
	}

	//显示按钮栏
	if(!this.ShowButtonRow){
		pw.$m("_ButtonRow_"+this.ID).hide();
	}
	if(this.CancelEvent){
...
Dialog.prototype.close = function(){
	if(this.innerElementId){
		var innerElement=$E.getTopLevelWindow().$m(this.innerElementId);
	...
	if(this.WindowFlag){
		...
		if(this.AlertFlag){
			$E.hide(pw.$m("_AlertBGDiv"));
		}
		if(!flag&&!this.AlertFlag){//此处是为处理弹出窗口被关闭后iframe立即被重定向时背景层不消失的问题
			pw.eval('window._OpacityFunc = function(){var w = $E.getTopLevelWindow();$E.hide(w.$m("_DialogBGDiv"));}');
			pw._OpacityFunc();
		}
...
Dialog.prototype.addButton = function(id,txt,func){
	...
	var pw = $E.getTopLevelWindow();
	pw.$m("_DialogButtons_"+this.ID).$T("input")[0].getParent("a").insertAdjacentHTML("beforeBegin",html);
	pw.$m("_Button_"+this.ID+"_"+id).onclick = func;
}
...
Dialog.alert = function(msg,func,w,h){
	...
	diag.show();
	pw.$m("_AlertBGDiv").style.display="";
	$E.hide(pw.$m("_ButtonOK_"+diag.ID));
	var win = pw.$m("_DialogFrame_"+diag.ID).contentWindow;
	...
	pw.$m("_DialogButtons_"+diag.ID).style.textAlign = "center";
}
...
Dialog.confirm = function(msg,func1,func2,w,h){
	...
	diag.show();
	pw.$m("_AlertBGDiv").style.dispaly="";
	var win = pw.$m("_DialogFrame_"+diag.ID).contentWindow;
	var doc = win.document;
	doc.open();
	doc.write("<body oncontextmenu='return false;'></body>") ;
	...
	if(d){
		try{
			...
			if(!this.AlertFlag){
				...
				if(!flag){
					$E.getTopLevelWindow().$m("_DialogBGDiv").style.opacity="0.4";
					$E.getTopLevelWindow().$m("_DialogBGDiv").style.filter="alpha(opacity=40)";
				}
			}
			if(d.AlertFlag){
				$E.show($E.getTopLevelWindow().$m("_AlertBGDiv"));
			}
...
Dialog.setPosition=function(){
	...
	{
		pw.$m("_DialogDiv_"+DialogArr[i]).DialogInstance.setPosition();
	}
}
Dialog.prototype.setPosition=function(){
	...
	pw.$m(this.bgdivID).style.height= sh + "px";
}
