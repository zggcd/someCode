...
function toAdd(){
    createNewTab('tabView1','新增策略组','','/supt/taticConfigAction.do?method=taticGroupDetail',true);
}
...
function selectElement(){
     var  returnval=window.showModalDialog("/jsp/supt/taticConfig/selectElementList.jsp?d="+new Date(),window,"scroll:yes;status:no;help:0;resizable:0;dialogWidth:800px;dialogHeight:500px");
     var elementCheckids="|"+getSelectCheckboxValue("detailForm","elementCheckid","|")+"|";
     if(returnval!=null&&returnval!="undefined"){
        var ids=returnval.split("`~`")[0];
        var names=returnval.split("`~`")[1];
        var id=ids.split("|");
        var name=names.split("|");
        var str=$('elementSpan').innerHTML;
        for(var i=0;i<id.length;i++){
           if(elementCheckids.indexOf("|"+id[i]+"|")==-1){
              str=str+"<input type='checkbox' name='elementCheckid' checked value="+id[i]+">"+name[i]+";";
          }
        }
        $('elementSpan').innerHTML = str;
     }
}
</script>
