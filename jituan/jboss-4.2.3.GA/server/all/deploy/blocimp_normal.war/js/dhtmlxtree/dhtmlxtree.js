...
This version of Software is free for using in GPL applications. For commercial use please contact info@dhtmlx.com to obtain license*/
    ...
    if (optionStr) {
        ...
        for (var i = 0; i < tempStr.length; i++) {
            switch (tempStr[i])
                ...
                this._setCheck(Nodes[Count], "unsure");break;case "OPEN": Nodes[Count].openMe = 1;break;}
            ;
        }
        ;
    }
...
dhtmlXTreeObject.prototype._parseXMLTree = function(dhtmlObject, node, parentId, level, xml_obj, start) {
    ...
    for (var i = start || 0; i < node.childNodes.length; i++) {
        if ((node.childNodes[i].nodeType == 1) && (node.childNodes[i].tagName == "item"))
        {
            ...
            if (achecked == -1)zST += ",HCHECKED"; else if (achecked)zST += ",CHECKED";
            if (aopen)zST += ",OPEN";
            if (dhtmlObject.waitUpdateXML) {
                if (dhtmlObject._globalIdStorageFind(cId))
                    var newNode = dhtmlObject.updateItem(cId, name, im0, im1, im2, achecked); else {
                    if (npl == 0)zST += ",TOP"; else preNode = temp.childNodes[npl];
                ...
                var newNode = dhtmlObject._attachChildNode(temp, cId, name, 0, im0, im1, im2, zST, chd, 0, preNode);
            if (tooltip)newNode.span.parentNode.parentNode.title = tooltip;
            if (style)if (newNode.span.style.cssText)newNode.span.style.cssText += (";" + style); else
    ...
    if (!level) {
        if (dhtmlObject.waitUpdateXML) {
            ...
            for (var i = temp.childsCount - 1; i >= 0; i--)if (temp.childNodes[i]._dmark)dhtmlObject.deleteItem(temp.childNodes[i].id);
        }
        ;
        var parsedNodeTop = dhtmlObject._globalIdStorageFind(dhtmlObject.parsingOn);
        for (var i = 0; i < dhtmlObject.parsedArray.length; i++)parsedNodeTop.htmlNode.childNodes[0].appendChild(dhtmlObject.parsedArray[i]);
        ...
        for (var n = 0; n < chArr.length; n++)if (chArr[n])dhtmlObject.setCheck(chArr[n], 1);
        if ((dhtmlObject.XMLsource) && (dhtmlObject.tscheck) && (dhtmlObject.smcheck) && (temp.id != dhtmlObject.rootId)) {
            if (temp.checkstate === 0)dhtmlObject._setSubChecked(0, temp); else if (temp.checkstate === 1)dhtmlObject._setSubChecked(1, temp);
        }
        ;
    ...
    if (dhtmlObject.parsCount == 1) {
        ...
        if ((!dhtmlObject._edsbps) || (!dhtmlObject._edsbpsA.length)) {
            window.setTimeout(function() {
                if (dhtmlObject.onXLE)dhtmlObject.onXLE(dhtmlObject, parentId);
...
dhtmlXTreeObject.prototype._epnFHe = function(that, id, flag) {
    ...
    if (!flag)if (that.onXLE)that.onXLE(that, id);
};
...
dhtmlXTreeObject.prototype.changeItemId = function(itemId, newItemId)
{
    ...
    for (var i = 0; i < this._globalIdStorageSize; i++)if (this._globalIdStorage[i] == itemId) {
...
dhtmlXTreeObject.prototype._openItem = function(item) {
    ...
    if ((state < 0) || (((this.XMLsource) && (!item.XMLload)))) {
        ...
        if (this.checkEvent("onOpenEnd")) {
            ...
            if (!this.xmlstate)this.callEvent("onOpenEnd", [item.id,this._getOpenState(item)]); else {
...
dhtmlXTreeObject.prototype.getAllCheckedItemText = function() {
    var str = "";
    $A(this._getAllChecked("", "", 1).split(this.dlmtr)).each(function (id){
        str =str+ tree.getItemText(id)+tree.dlmtr;
    });
