Validator = {
    ...
    IPPORT : /^(([01]?\d\d?)|(2[0-4]\d)|(25[0-5]))(\.(([01]?\d\d?)|(2[0-4]\d)|(25[0-5]))){3}(:\d{1,5})?$/,
    ...
    DateTime2 : /^(\d{1,4})(-|\/)(\d{1,2})\2(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})$/,
    ...
    Validate : function(theForm, mode) {
        ...
        for (var i = 0; i < count; i++) {
            with (obj.elements[i]) {
                ...
                if (getAttribute("require") == "false" && value == "" && _dataType!="Require") continue;
                switch (_dataType) {
                    case "IdCard" :
                    case "Date" :
                    case "Repeat" :
...
/**
 * 验证表单,obj为需要验证的表单对象,mod为验证方式
 */
function validateForm(obj, mod) {
    if (mod && (mod == 1 || mod == 2)) {
        return Validator.Validate(obj, mod);
    } else {
