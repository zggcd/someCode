...
(function($){
	...
	function init(target){
		...
		var tables = $(target).children("table.first");
		var table = $("#portallayout tbody");
		//var tr = table.find('tr');
		var columnWidths = [];
		var totalWidth = 0;
		//$(target).children('div:first').addClass('portal-column-left');
		//$(target).children('div:last').addClass('portal-column-right');
		$(table).find('>tr').each(function(){	// each column panel
			var tr = $(this);
		
			tr.find('>td').each(function(){
				var cname =  $(this).attr('class');
               if( 'portal-column-td' != cname)
				{
				  return true; 
			   }
				...
				var column = $(this).find(">div").first();
        ...
        /*
		var tr2 = $('<tr></td>').appendTo(table);
        var td2 = $('<td colspan="2" class="portal-column-td"></td>').appendTo(tr2);
         var div2 = $('<div class="portal-column"></div>').appendTo(td2);
		var px = $('<div />').appendTo(div2);
				px.addClass('portal-p').panel({
					title:'添加',
           ...
           px.addClass('portal-p');
		   makeDraggable(target, px);
		   */
			//$('#pp').portal('enableDragging',px);
	...
	$.fn.portal.methods = {
		...
		add: function(jq, param){	// param: {panel,columnIndex}
			return jq.each(function(){
				...
				p.panel('panel').addClass('portal-panel').insertBefore(c);
