...
public final class moduleNavigationXMLData_jsp extends org.apache.jasper.runtime.HttpJspBase
  ...
  public void _jspService(HttpServletRequest request, HttpServletResponse response)
	...
	try {
		...
		hashtable.put("itemCode","SYS_SSO_KEY");
		hashtable.put("ServletContext", sc);
		Hashtable returnValue = (Hashtable) business.invoke("baf_commonBI","getParameterValue", hashtable);
		String ssokey="",appCode="";
		if(returnValue!=null){
			ssokey = (String) returnValue.get("getParameterValue");
		}
		hashtable=new Hashtable();
		hashtable.put("itemCode","SYS_APP_CODE");
		hashtable.put("ServletContext", sc);
		returnValue = (Hashtable) business.invoke("baf_commonBI","getParameterValue", hashtable);
		
		if(returnValue!=null){
			appCode = (String) returnValue.get("getParameterValue");
		}
		...
		MD5 md5 = new MD5();
		ssokey= md5.getMD5ofStr(loginName + ssokey);//外部系统的sso key
		...
		if(list!=null && list.size()>0){
			for (int i = 0; i < list.size(); i++) {
				...
				if (k == 1 && type.equalsIgnoreCase("0")){
					buf.append(open);
				}
				...
				if ("Y".equalsIgnoreCase(vo.getIsLeaf())){
				    String appId=StringHelper.convertStringNull(vo.getApplicationId());
				    //当模块链接到外部系统时，需要传递operID和hashCode两个参数
		        	if(!"".equals(appId)&& !appCode.equals(appId)){
		        		url = url + (url.indexOf("?")==-1?"?":"&")+"operID="+loginName+"&hashCode="+ssokey;
		        	}
					tempType = "M";
				}
